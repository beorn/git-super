...
async function cleanSnapshot(
  ...
  if (status.records.length > 0) throw new Error(`worktree ${path} is dirty: ${status.records.join("; ")}; preserve its changes before removal`)
  ...
  for (const entry of status.consultedRepositories) {
    ...
    if (!state.registered) throw new Error(`worktree ${entry.root} is absent from its Git registration; inspect git worktree list`)
    if (state.locked !== undefined) throw new Error(`worktree ${entry.root} is locked: ${state.locked}; resolve its holder before removal`)
    const dirty = await git.text(entry.root, ["status", "--porcelain=v1", "-z", "--untracked-files=all", "--ignore-submodules=none"])
...
function manifest(root: string): Readonly<Record<string, string>> {
  ...
  for (const entry of readdirSync(root, { recursive: true, withFileTypes: true })) {
    ...
    if (!entry.isFile()) throw new Error(`Git store ${path} is not a regular file; preserve and resolve it before removal`)
...
export async function retainWorktreeModules(
  ...
  if (!registered.registered) throw new Error(`worktree ${path} is not registered in ${repo}; inspect git worktree list`)
  if (registered.locked !== undefined) throw new Error(`worktree ${path} is locked: ${registered.locked}; resolve its holder before removal`)
  ...
  if (gitDir === common) throw new Error(`worktree ${path} is the primary worktree; only a linked worktree can be removed`)
  ...
  if (within(path, canonical) || within(gitDir, canonical)) {
    throw new Error(`retention directory ${canonical} is inside worktree removal paths ${path} or ${gitDir}; choose an external durable directory`)
  }
  ...
  if (retained !== null) {
    ...
    const compared = spawnSync("diff", ["-r", "--", modules, retained], { encoding: "utf8", timeout: 120_000, maxBuffer: 1024 * 1024 })
    if (compared.error || compared.status !== 0) {
      throw new Error(`retention comparison failed for ${modules} and ${retained}: ${compared.error?.message || compared.stderr || compared.stdout || `exit ${String(compared.status)}`}; worktree preserved`)
    }
    if (JSON.stringify(manifest(retained)) !== JSON.stringify(hashes) || JSON.stringify(manifest(modules)) !== JSON.stringify(hashes)) {
      throw new Error(`Git store ${modules} changed during retention at ${retained}; worktree preserved, retry after its writer stops`)
    }
  }
  const after = await cleanSnapshot(git, path, inspect)
  if (JSON.stringify(after) !== JSON.stringify(before)) throw new Error(`worktree ${path} changed during retention; preserved, retry after its writer stops`)
